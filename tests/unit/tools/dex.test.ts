...
describe("DEX Tool", () => {
  ...
  it("does not throw on import when STELLAR_PUBLIC_KEY is missing", async () => {
    delete process.env.STELLAR_PUBLIC_KEY;
    vi.doMock("../../../lib/dex", () => ({
      quoteSwap: vi.fn(),
      swapBestRoute: vi.fn(),
    }));
    await expect(import("../../../tools/dex")).resolves.toBeDefined();
  });

  it("throws at call time when STELLAR_PUBLIC_KEY is missing", async () => {
    delete process.env.STELLAR_PUBLIC_KEY;
    vi.doMock("../../../lib/dex", () => ({
      quoteSwap: vi.fn(),
      swapBestRoute: vi.fn(),
    ...
    ).rejects.toThrow("Missing STELLAR_PUBLIC_KEY environment variable");
  });
